...
export default function PatientProfile() {
  ...
  const patientChildren = children.filter(c => c.beneficiaryId === id);
  ...
  const handleEnroll = async (schemeId: string) => {
    ...
    try {
      await enrollBeneficiary(schemeId, patient.id, currentUser.id);
  ...
  const handleSaveProfile = async () => {
    await updateBeneficiaryProfile(patient.id, formData);
    setEditMode('none');
  ...
  const handleDeleteUser = async () => {
    if (confirm(`Are you sure you want to delete ${patient.name}? This action cannot be undone.`)) {
      await deleteBeneficiary(patient.id);
      ...
      <main className="p-4 space-y-6">
        ...
        {activeTab === 'history' && (
          <section className="space-y-4">
             <Button onClick={() => navigate(`/asha/visit?patientId=${patient.id}`)} className="w-full bg-teal-600 hover:bg-teal-700">
