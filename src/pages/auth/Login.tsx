      ...
      if (isLoginMode) {
        ...
        if (data.user) {
          ...
          if (profileError) {
             ...
             login(newUser as UserType);
          } else {
             login(userProfile as UserType);
          }
        ...
        if (data.user) {
          if (data.session) {
            login({ id: data.user.id, name: formData.name, role } as UserType);
            addToast(t('common.success'), 'success');
            if (role === 'beneficiary') navigate('/beneficiary');
            else if (role === 'asha_worker') navigate('/asha');
            else navigate('/partner');
          } else {
            addToast('Verification email sent!', 'info', 6000);
            setIsLoginMode(true);
          }
        }
      }
